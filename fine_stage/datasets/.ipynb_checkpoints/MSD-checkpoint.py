...
class MSDDataset(Dataset):
    def __init__(self, cfg, root_dir, transform=None, if_self_training=False):
        ...
        #     images = [images[i] for i in range(0, total_images, step)]

        self.images = images
        
        # if self.images[0].endswith("jpg"):
        self.gts = [image_path.replace("image", "mask").replace("jpg", "png") for image_path in self.images]
        # else:
        #     self.gts = [image_path.replace("image", "mask") for image_path in self.images]
...
class MSDDatasetwithCoarse(MSDDataset):
...
def load_datasets(cfg, img_size):
    ...
    val = MSDDataset(
        cfg,
        root_dir=cfg.datasets.MSD.test_real,
        transform=transform,
    )
    train = MSDDataset(
        cfg,
        root_dir=cfg.datasets.MSD.train_real,
        transform=transform,
