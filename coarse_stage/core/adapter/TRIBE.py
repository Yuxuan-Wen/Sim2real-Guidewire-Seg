...
class TRIBE_class(BaseAdapter):
    def __init__(self, model, optimizer):
        super(TRIBE_class, self).__init__(model, optimizer)
    ...
    def update_model(self, model, optimizer, batch_data, logit):
        ...
        entropy_mask = (entropy < 0.05 * math.log(2))
        
        
        # print(stu_sup_out.shape, ema_sup_out.shape)
        # # torch.Size([1, 2, 1024, 1024]) torch.Size([1, 2, 1024, 1024])
        # print(entropy_mask.shape)

        l_sup = torch.nn.functional.cross_entropy(stu_sup_out, ema_sup_out.argmax(dim=1), reduction='none')
        # l_sup = torch.nn.functional.cross_entropy(stu_sup_out, ema_sup_out.argmax(dim=-1), reduction='none')[entropy_mask].mean()
        
        
        l_sup = l_sup[entropy_mask].mean()

        with torch.no_grad():
            self.set_bn_label(self.source_model, p_l)
            source_anchor = self.source_model(batch_data).detach()
        
        l_reg = 0.5 * torch.nn.functional.mse_loss(ema_sup_out, source_anchor, reduction='none').argmax(dim=1)
        
        print(l_reg.shape)
        print(entropy_mask.shape)
        
        l_reg = l_reg[entropy_mask].mean()
